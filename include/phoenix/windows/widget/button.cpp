...
void pButton::setImage(const image& image, Orientation orientation) {
  ...
  switch(orientation) {
  case Orientation::Horizontal: SetRect(&list.margin, 0, 0, 0, 0); list.uAlign = BUTTON_IMAGELIST_ALIGN_LEFT; break;
  case Orientation::Vertical:   SetRect(&list.margin, 0, 0, 0, 0); list.uAlign = BUTTON_IMAGELIST_ALIGN_TOP;  break;
  }
